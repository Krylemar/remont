...
<body>
	...
	<?php 
		if (isset($_POST['run'])) {
			...
			$sql = "SELECT r.id_repair, r.date, r.price, e.first_name AS fname, e.last_name AS lname
					FROM `Repairs` AS r 
					JOIN `Employees` AS e ON r.employee = e.id_employee
					WHERE r.client = '$id'";
			$result = $dbConn->query($sql);
			$employee_names_result = $dbConn->query("SELECT first_name AS fname, last_name AS lname FROM `Clients` WHERE id_client = '$id'");
